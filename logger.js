...
// Derive wallet tag fresh each call (supports runtime WALLET_PRIVATE_KEY switching)
function walletTag() {
  try {
    if (process.env.WALLET_PRIVATE_KEY) {
      return deriveAddress(process.env.WALLET_PRIVATE_KEY).slice(0, 12);
    }
  } catch {}
  return null;
}
...
export function log(category, message) {
  ...
  const tag = walletTag();
  const line = `[${timestamp}]${tag ? ` [${tag}]` : ""} [${category.toUpperCase()}] ${message}`;
