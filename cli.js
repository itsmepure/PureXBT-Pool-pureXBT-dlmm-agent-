#!/usr/bin/env node
/**
 * pureXBT — Solana DLMM LP Agent CLI
...
// ─── Load .env from ~/.purexbt/ if present ──────────────────────
const purexbtDir = path.join(os.homedir(), ".purexbt");
const purexbtEnv = path.join(purexbtDir, ".env");
if (fs.existsSync(purexbtEnv)) {
  loadEnv({
    envPath: purexbtEnv,
    keyPath: path.join(purexbtDir, ".envrypt"),
    override: false,
  });
}
...
// ─── SKILL.md generation ──────────────────────────────────────────
const SKILL_MD = `# pureXBT — Solana DLMM LP Agent CLI
...
### pureXBT study --pool <addr> [--limit 4]
...
### pureXBT token-info --query <mint_or_symbol>
...
### pureXBT token-holders --mint <addr> [--limit 20]
...
fs.mkdirSync(purexbtDir, { recursive: true });
fs.writeFileSync(path.join(purexbtDir, "SKILL.md"), SKILL_MD);
...
switch (subcommand) {
  ...
  case "pnl": {
    ...
    if (!positionAddress) die("Usage: pureXBT pnl <position_address>");
  ...
  case "token-info": {
    ...
    if (!query) die("Usage: pureXBT token-info --query <mint_or_symbol>");
  ...
  case "token-holders": {
    ...
    if (!mint) die("Usage: pureXBT token-holders --mint <addr>");
  ...
  case "token-narrative": {
    ...
    if (!mint) die("Usage: pureXBT token-narrative --mint <addr>");
  ...
  // ── pool-detail ───────────────────────────────────────────────
  case "pool-detail": {
    if (!flags.pool) die("Usage: pureXBT pool-detail --pool <addr> [--timeframe 5m]");
  ...
  case "search-pools": {
    ...
    if (!query) die("Usage: pureXBT search-pools --query <name_or_symbol>");
  ...
  // ── active-bin ────────────────────────────────────────────────
  case "active-bin": {
    if (!flags.pool) die("Usage: pureXBT active-bin --pool <addr>");
    const { getActiveBin } = await import("./tools/dlmm.js");
  ...
  case "wallet-positions": {
    ...
    if (!wallet) die("Usage: pureXBT wallet-positions --wallet <addr>");
  ...
  // ── deploy ───────────────────────────────────────────────────────
  case "deploy": {
    if (!flags.pool) die("Usage: pureXBT deploy --pool <addr> --amount <sol>");
  ...
  // ── claim ────────────────────────────────────────────────────────
  case "claim": {
    if (!flags.position) die("Usage: pureXBT claim --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── close ────────────────────────────────────────────────────────
  case "close": {
    if (!flags.position) die("Usage: pureXBT close --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── swap ─────────────────────────────────────────────────────────
  case "swap": {
    if (!flags.from || !flags.to || !flags.amount) die("Usage: pureXBT swap --from <mint> --to <mint> --amount <n>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  case "config": {
    if (sub2 === "get" || !sub2) {
      ...
      if (!key || rawVal === undefined) die("Usage: pureXBT config set <key> <value>");
      let value = rawVal;
  ...
  // ── study ────────────────────────────────────────────────────────
  case "study": {
    if (!flags.pool) die("Usage: pureXBT study --pool <addr> [--limit 4]");
  ...
  case "start": {
    ...
    process.stderr.write("[pureXBT] Starting autonomous agent...\n");
    startCronJobs();
    break;
  }
  ...
  case "lessons": {
    if (sub2 === "add") {
      ...
      if (!text) die("Usage: pureXBT lessons add <text>");
  ...
  // ── pool-memory ──────────────────────────────────────────────────
  case "pool-memory": {
    if (!flags.pool) die("Usage: pureXBT pool-memory --pool <addr>");
  ...
  case "blacklist": {
    if (sub2 === "add") {
      if (!flags.mint) die("Usage: pureXBT blacklist add --mint <addr> --reason <text>");
  ...
  case "withdraw-liquidity": {
    if (!flags.position) die("Usage: pureXBT withdraw-liquidity --position <addr> --pool <addr> [--bps 10000]");
  ...
  // ── add-liquidity ──────────────────────────────────────────────
  case "add-liquidity": {
    if (!flags.position) die("Usage: pureXBT add-liquidity --position <addr> --pool <addr> [--amount-x <n>] [--amount-y <n>]");
    if (!flags.pool) die("--pool is required");
  ...
  default:
    die(`Unknown command: ${subcommand}. Run 'pureXBT help' for usage.`);
}
