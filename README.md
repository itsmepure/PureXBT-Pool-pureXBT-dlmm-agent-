...
- **Indicator-exit gates** — the bb_plus_rsi exit signal was firing minutes after momentum entries (the exit condition ≈ the entry condition on a fresh breakout), closing brand-new positions at near-zero PnL. It now only evaluates positions older than `exitMinAgeMin` (default 30) with PnL above `exitMinPnlPct` (default 2, ships at 0.5 in the example) — both hot-reloadable
- **Momentum-death exit tamed** — three changes: (1) it no longer fires while the position is out-of-range ABOVE (the position is ~100% SOL there, so there is nothing to protect — and closing early was cancelling chase-up); (2) the raw signal must persist for `momentum.exit.confirmTicks` consecutive scans (default 3 ≈ 1 minute) instead of a single 20s snapshot; (3) the trigger is now AND (sellers dominant AND price falling) instead of OR
- **Minimum token age gate** — deploys into tokens younger than `minTokenAgeHours` (default 3h) are rejected at two layers: the momentum prescreen (new `age`/`ageMissing` reject buckets, zero extra API calls) and the deploy validator (all deploy paths, fail-closed). Age comes from the OLDEST DexScreener `pairCreatedAt` across the token's pairs — a proxy for launch time that a newer, more liquid pair can't fake
- **Held close-card watcher** — close notifications whose reason mentions `chase_up` are intentionally held while a chase redeploy is pending, but closes issued outside the chase pipeline (e.g. the management LLM echoing chase context in its reason) fell into that hold with no fallback and vanished. Every held card now gets a 6-minute watcher: if the pool has no new position by then, the card is sent
- **Self-tuning momentum exit** — every momentum-exit close snapshots the exit price and re-samples it 30 minutes later, classifying the exit as `good_exit` (price dumped — the exit saved capital), `premature` (price kept pumping — we left a healthy runner) or `neutral`, appended to `logs/momentum-exit-outcomes.jsonl`. A 12-hour tuner cron feeds the aggregate to the LLM, which may adjust the three exit knobs via new clamped `update_config` keys (`momentumExitBuysVsSellsBelow` 0.5–1.2, `momentumExitPriceChangeBelow` −5..0, `momentumExitConfirmTicks` 1–10) — at most one small step per run, only with ≥5 samples and clear evidence, with an automatic `[SELF-TUNED]` lesson
- **Single-message reshape notification with PnL card** — a successful reshape (chase close → redeploy) previously produced no close notification at all for the old position, then briefly two separate messages. It now sends exactly ONE message: the full PnL card PNG of the closed position with the reshape status as the first caption line ("✅ RESHAPE BERHASIL" on success, "🛑 RESHAPE GAGAL / TIDAK JADI" when the redeploy is blocked or the LLM judges momentum broken) — sent immediately when the redeploy resolves, not after the 6-minute watcher
- **User-only config lock** — position sizing and risk keys (`deployAmountSol`, `maxDeployAmount`, `positionSizePct`, `gasReserve`, `maxPositions`, `takeProfitPct`, `stopLossPct`, `hardStopLossPct`, `trailingTriggerPct`, `trailingDropPct`, `feeSplitUsdcPct`) can no longer be changed by the agent: `update_config` rejects them with an explicit "user-only" error unless the call comes from a user surface (dashboard / Telegram settings / `/setcfg`), which passes a one-shot in-process grant that tool-calling LLMs cannot trigger
