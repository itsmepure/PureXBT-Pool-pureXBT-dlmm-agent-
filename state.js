...
export function trackPosition({
  ...
  state.positions[position] = {
    ...
    wallet: deriveAddress(process.env.WALLET_PRIVATE_KEY || "") || null,
...
export function syncOpenPositions(active_addresses) {
  ...
  const currentWallet = deriveAddress(process.env.WALLET_PRIVATE_KEY || "");
  let changed = false;
  ...
  for (const posId in state.positions) {
    ...
    // Only auto-close positions owned by THIS wallet (prevents cross-wallet contamination)
    if (currentWallet && pos.wallet && pos.wallet !== currentWallet) {
      log("state", `Position ${posId} belongs to ${pos.wallet?.slice(0, 8)} (not ${currentWallet?.slice(0, 8)}) — skipping auto-close`);
      continue;
    }
